...
- **Withdraw in-flight entries a denied filter earned**, and only those that would put a *payload*
  back on the wire. An entry past its PUBREC owes a PUBREL, which carries no payload; withholding
  it would leave the client waiting for a PUBCOMP forever and its Packet Identifier unusable, to
  protect a disclosure that already happened.

  "Matches no surviving filter" is *not* the same test, and it was the first draft's — review
  caught it. `handleUnsubscribe` deliberately leaves the in-flight set alone, so a message earned
  by a filter the client has since unsubscribed from matches nothing live and is still one the
  broker MUST resend [MQTT-4.4.0-1]. The narrower test is "a denied filter matches it and no
  surviving one does".
- **Drop the deliveries already queued for the resuming connection.** `sess.attach` points the
  session's NATS handlers at this connection *before* `reauthoriseLive` runs, so traffic on a
  still-live denied subscription is buffered in `c.deliveries` for the whole duration of the
  `Authorizer` call — a window the client sizes, by choosing when to reconnect and how many
  filters to hold. `deliverLoop` would then write it out right after the CONNACK. One message can
  still slip through behind the drain, from a dispatcher already inside `onNATSMessage`; closing
  that means checking every delivery against the session on the hot path, which is not a trade
  this entry should make unmeasured.
...
None. The change is confined to `handshake.go`, `session.go`, `publish.go` and `subscribe.go`, and
adds `topic.MatchAny` next to `topic.Match`, which already implements MQTT-5.0 §4.7 filter matching
including the shared-filter rule from §4.8.2.

`AuthzRequest` gains a `Resume bool`. Its zero value is exactly today's meaning at the two existing
call sites, so the addition is invisible to an implementation that ignores it — and an
implementation that audit-logs, meters or rate-limits subscriptions needs it, because a resumed
filter is not an action the client performed.

`session.identity` and `session.username` move behind `setPrincipal`/`principal`. They were written
unguarded at handshake time and read from other goroutines on every authorisation decision, which
is the one race that could make a decision against the wrong principal. Pre-existing, found by
review of this change because this change adds a third reader.
...
- **A transient `Authorizer` failure is indistinguishable from a denial, and here a denial is
  destructive.** The interface returns one `error` and `mayResume` reads any of them as "no". On a
  SUBSCRIBE that costs a 0x87 and a retry; on a resume it tears the filter down and, with
  persistence, writes the reduced set back to the durable record. A policy service returning 500
  during a reconnect storm therefore unsubscribes a fleet permanently. Documented on `Authorizer`
  and filed as a roadmap entry with the shape of the fix; not fixed here, because an exported
  sentinel is a policy decision the doctrine says belongs in its own change.

## What review caught that the tests did not

Four reviews ran against the diff before it was published, and three findings were defects rather
than opinions. Recording them because each is a case of code that was correct-looking and green:

- The withdrawal scope, above. The end-to-end tests all passed; none of them unsubscribed.
- `dropQueued` did not exist. The doc comment claimed a denied filter "stops delivering before the
  client is told its session is present", which was true of the delivery *goroutine* and false of
  the queue it drains.
- `nextID` did not skip a withdrawn identifier (see the trade-offs above).

Mutation testing then found three behaviours that were right and unproven — deleting the code left
the whole suite green. Each now has a test that kills the mutant: the `handlePubcomp` withdrawal
branch, the ordering of `reauthoriseLive` before `persistSession`, and `dropQueued` itself. The
last two needed a specific shape to discriminate at all; a clean disconnect and restart passes the
ordering test either way, because `releaseStoredSession` rewrites the record at close.
